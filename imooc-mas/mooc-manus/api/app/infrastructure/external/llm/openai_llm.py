        ...
        try:
            ...
            if tools:
                ...
                response = await self._client.chat.completions.create(
                    ...
                    parallel_tool_calls=False,  # 关闭并行工具调用
                    timeout=self._timeout
                )
            else:
                # 2.没有传递工具则删除Tools/tool_choice等参数
                logger.info(f"调用OpenAI客户端向LLM发起请求没有携带工具: {self._model_name}")
                response = await self._client.chat.completions.create(
                    model=self._model_name,
                    temperature=self._temperature,
                    max_tokens=self._max_tokens,
            ...
            raise ServerError("调用OpenAI客户端向LLM发起请求出错")
